module ItemsHelper
  ...
  def items_archived ids
    notice = <<-HTML
     ...
     <p><a href='items/undo_actions?ids=#{ids.join(",")}&archived=true&page=#{params[:page]}'  data-remote="true">Undo this action</a> to move archived items back to active.</p>
    HTML
    notice = notice.html_safe
  end
  ...
  def items_deleted ids
    notice = <<-HTML
     ...
     <p><a href='items/undo_actions?ids=#{ids.join(",")}&deleted=true&page=#{params[:page]}'  data-remote="true">Undo this action</a> to move deleted items back to active.</p>
    HTML
    notice = notice.html_safe
  end
